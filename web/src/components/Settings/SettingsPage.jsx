...
export default function SettingsPage({ user, onUpdate }) {
  ...
  const handleModalInputChange = (val) => {
    ...
    if ((activeModalField === 'firstName' || activeModalField === 'lastName') && val.trim() !== '') {
      if (!/^[A-Za-zÀ-ÿ\s'-]+$/.test(val)) {
        errorMsg = 'Letters, hyphens, or spaces only.';
      }
    }
    if (activeModalField === 'phone' && val.trim() !== '') {
      if (!/^\+?[0-9\s-]*$/.test(val)) {
  ...
  const handleUpdateField = async () => {
    if (modalError || !modalInputValue.trim()) return;
    setLoading(true);
    setMessage('');
    try {
      const updatePayload = {
  ...
  const requestVerificationToken = async (e) => {
    ...
    try {
      ...
      setSecurityStatus({ type: 'success', message: 'Verification code sent to your email.' });
    } catch (err) {
      setSecurityStatus({ type: 'error', message: err.response?.data?.message || 'Could not send verification code.' });
    }
  };
  ...
  const verifyTokenHandler = async (e) => {
    ...
    try {
      ...
      setSecurityStatus({ type: 'success', message: 'Code verified. You can now change your password.' });
    } catch (err) {
      setSecurityStatus({ type: 'error', message: err.response?.data?.message || 'Invalid verification code.' });
    }
  };
  ...
  const executePasswordChange = async (e) => {
    ...
    try {
      ...
      setSecurityStatus({ type: 'success', message: data.message || data.msg || 'Password changed successfully.' });
      setVerifiedForPasswordChange(false);
      setRequestedCode(false);
      setVerificationCode('');
      setPasswords({ currentPassword: '', newPassword: '', confirmPassword: '' });

      setTimeout(() => setSecurityStatus({ type: '', message: '' }), 5000);
    } catch (err) {
      setSecurityStatus({ type: 'error', message: err.response?.data?.message || 'Failed to change password.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="max-w-4xl mx-auto p-4 space-y-6 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <section className="flex flex-col items-center justify-center p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl text-center shadow-xs">
      ...
      <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xs space-y-4">
        ...
        {securityStatus.message && (
          <div className={`p-4 rounded-2xl flex items-center space-x-2 text-xs font-bold border ${securityStatus.type === 'success' ? 'bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 border-emerald-200' : 'bg-red-50 dark:bg-red-950/20 text-red-700 dark:text-red-400 border-red-200'}`}>
            <AlertCircle size={14} />
        ...
        {!requestedCode ? (
          ...
          <form onSubmit={executePasswordChange} className="space-y-4 max-w-sm">
            ...
            <div>
              ...
              <div className="relative flex items-center">
                ...
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 p-1 rounded-md text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors focus:outline-hidden">
                  {showPass ? (
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4"><path fillRule="evenodd" d="M3.28 2.22a.75.75 0 00-1.06 1.06l14.5 14.5a.75.75 0 101.06-1.06l-1.745-1.745a10.029 10.029 0 003.3-4.38.75.75 0 000-.536 10.048 10.048 0 00-11.16-6.73L3.28 2.22zM8.22 5.4a5.13 5.13 0 013.561 1.255L10.23 8.207a2.25 2.25 0 00-2.01 2.01L6.666 8.663A5.111 5.111 0 018.22 5.4z" clipRule="evenodd" /><path d="M12.423 14.547a8.536 8.536 0 01-4.203.953A8.503 8.503 0 011 10c0-.687.082-1.354.238-1.996l2.155 2.156a3.75 3.75 0 004.49 4.49l2.54 2.541z" /></svg>
                  ) : (
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4"><path d="M10 12.5a2.5 2.5 0 100-5 2.5 2.5 0 000 5z" /><path fillRule="evenodd" d="M.664 10.59a1.651 1.651 0 010-1.18 10.051 10.051 0 0118.673 0 1.651 1.651 0 010 1.18 10.051 10.051 0 01-18.672 0zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clipRule="evenodd" /></svg>
                  )}
                </button>
              </div>
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs transition shadow-xs">
      ...
      <section className="bg-white dark:bg-slate-900 border border-rose-200 dark:border-rose-950/40 rounded-3xl p-6 shadow-xs space-y-3">
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 font-bold leading-relaxed">Deleting your account is permanent. All profile data will be completely removed.</p>
      ...
      <Modal isOpen={activeModalField !== null} onClose={() => setActiveModalField(null)} title={`Update ${activeModalField}`}>
        <div className="space-y-4 pt-2 dark:text-slate-100">
          {activeModalField === 'gender' ? (
            <select value={modalInputValue} onChange={e => handleModalInputChange(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-bold outline-none text-slate-900 dark:text-slate-100">
            ...
            <div>
              <StandardTextInput type="text" value={modalInputValue} onChange={e => handleModalInputChange(e.target.value)} placeholder="Enter new value..." />
              {modalError && <p className="mt-1.5 text-xs font-semibold text-rose-600">{modalError}</p>}
            </div>
          )}
          <div className="flex gap-2 justify-end text-xs font-bold">
            ...
            <button onClick={handleUpdateField} disabled={loading || !!modalError || !modalInputValue.trim()} className="px-4 py-2 text-white bg-slate-900 dark:bg-emerald-600 rounded-lg transition disabled:opacity-50">Confirm</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Delete Account">
        <div className="space-y-4 pt-2">
          <p className="text-xs font-bold text-slate-500 dark:text-slate-400 leading-relaxed">This will permanently delete your account. This action cannot be undone.</p>
